...
pub(super) fn ApplicationSecrets() -> impl IntoView {
    ...
    <fieldset disabled=move ||context.blocked() || context.saved.get().application.spec.manifest.is_some()>
        <For each={move ||context.saved.get().application.spec.services.into_iter().map(|s|s.name).collect::<Vec<_>>()} key=|name|name.clone() children=move |name|view!{<SecretFiles service_name=name/>}/>
    </fieldset>}
}
...
fn SecretFiles(service_name: String) -> impl IntoView {
    ...
    let save = move |_| {
        ...
        if let Some(target) = manifest
            ...
            .services
            .iter_mut()
            .find(|s| s.name == service.get_value())
        {
