...
impl SqliteStore {
    ...
    async fn operation_on(
        ...
        sqlx::query_as!(OperationRow,
            r#"SELECT id AS "id!",application_id,kind,state,generation,attempt,consecutive_failures,error_code,error_message,created_at_ms,updated_at_ms,started_at_ms,finished_at_ms FROM operations WHERE id=?1"#, id)
    ...
    pub async fn latest_operation_for_application(
        ...
        let id = id.as_str();
        sqlx::query_as!(OperationRow,
            r#"SELECT id AS "id!",application_id,kind,state,generation,attempt,consecutive_failures,error_code,error_message,created_at_ms,updated_at_ms,started_at_ms,finished_at_ms FROM operations WHERE application_id=?1 ORDER BY created_at_ms DESC,id DESC LIMIT 1"#, id)
    ...
    pub async fn retry_operation(&self, operation: &Operation) -> Result<Operation, StoreError> {
        ...
        let changed = sqlx::query!("UPDATE operations SET state='requested',error_code=NULL,error_message=NULL,started_at_ms=NULL,finished_at_ms=NULL,updated_at_ms=?1 WHERE id=?2 AND (state IN ('succeeded','failed','cancelled') OR (state='running' AND error_code IS NOT NULL)) AND id=(SELECT latest.id FROM operations latest WHERE latest.application_id=operations.application_id ORDER BY latest.created_at_ms DESC,latest.id DESC LIMIT 1)",now,operation.id)
            .execute(&mut *tx).await.map_err(StoreError::database)?.rows_affected();
        if changed != 1 {
            let current = Self::operation_on(&mut tx, &operation.id).await?;
    ...
    pub async fn recover_interrupted(&self) -> Result<u64, StoreError> {
        let now = now_ms();
        let mut tx = self.begin_immediate().await?;
        sqlx::query!("INSERT INTO events(application_id,operation_id,generation,attempt,kind,message,created_at_ms) SELECT application_id,id,generation,attempt,'operation_interrupted','daemon restarted during execution',?1 FROM operations WHERE state='running'",now).execute(&mut *tx).await.map_err(StoreError::database)?;
    ...
    pub(super) async fn insert_operation(
        ...
        sqlx::query!("INSERT INTO events(application_id,operation_id,generation,attempt,kind,message,created_at_ms) SELECT application_id,id,generation,attempt,'operation_cancelled','superseded by newer intent',?1 FROM operations WHERE application_id=?2 AND state IN ('requested','running')",now,app_id).execute(&mut **tx).await.map_err(StoreError::database)?;
