    ...
    pub(super) async fn deliver_notifications(
        ...
        loop {
            ...
            let result=async {
                ...
                let Some((delivery,destination))=self.store.claim_delivery().await? else {return Ok::<_,StoreError>(());};
                let event=match self.store.event(delivery.event_id).await {Ok(e)=>e,Err(StoreError::NotFound)=>return Ok(()),Err(e)=>return Err(e)};
                let payload=match destination.kind {
                    WebhookKind::Json=>serde_json::json!({"version":1,"instance_id":self.store.instance_id(),"delivery_id":delivery.id,"category":delivery.category,"event":event}),
                    WebhookKind::Discord=>{
                        let summary=event.message.as_deref().unwrap_or(&event.kind);
                        let content=format!("piqueld · {}\n{}\nApplication: {}\nEvent: {} · Delivery: {}{}",delivery.category,summary,event.application_id.as_ref().map_or_else(||"daemon".to_owned(),ToString::to_string),event.id,delivery.id,event.diagnostic.as_ref().map_or_else(String::new,|d|format!("\nDiagnostic: {}\n{}",d.id,d.next_action)));
                        serde_json::json!({"content":content.chars().take(1900).collect::<String>(),"allowed_mentions":{"parse":[]}})
                    }
                };
                let result=tokio::select! {()=cancellation.cancelled()=>return Ok(()),result=client.post(&destination.url).header("Idempotency-Key",&delivery.id).json(&payload).send()=>result};
                let delay=5_u64.saturating_mul(1_u64<<u32::try_from(delivery.attempts.saturating_sub(1).min(10)).unwrap_or(10)).min(3600);
                let (state,message,delay)=match result {
                    Ok(response) if response.status().is_success()=>("delivered",None,0),
                    Ok(response)=>{
                        let status=response.status();
                        let retry=status.is_server_error() || status==reqwest::StatusCode::TOO_MANY_REQUESTS || status==reqwest::StatusCode::REQUEST_TIMEOUT;
                        let retry_after=response.headers().get("retry-after").and_then(|v|v.to_str().ok()).and_then(|v|v.parse::<u64>().ok()).map_or(delay,|v|v.clamp(1,3600));
                        (if retry{"pending"}else{"failed"},Some(format!("Receiver returned HTTP {status}")),retry_after)
                    }
                    Err(error)=>("pending",Some(if error.is_timeout(){"Webhook request timed out"}else if error.is_connect(){"Could not connect to webhook receiver"}else{"Webhook transport failed"}.to_owned()),delay),
                };
                self.store.complete_delivery(&delivery.id,state,message.as_deref(),delay).await
            }.await;
            if let Err(error) = result {
        ...
        tokio::time::timeout(Duration::from_secs(8), async {
            loop {
                ...
                if deliveries
                    .items
                    .first()
                    .is_some_and(|d| d.state == "delivered")
                {
                    break;
                }
