...
pub(super) async fn list(
    ...
    Ok(ok(state.store.secrets(&ApplicationId::parse(id)?).await?))
}
...
pub(super) async fn delete(
    ...
    let _guard = state.store.secret_deletion_guard().await;
    let versions = state
        .store
        .secret_deletion_versions(&id, &name, generation)
        .await?;
    state.runtime.remove_secrets(&id, &versions).await?;
    state.store.delete_secret_rows(&id, &name).await?;
    Ok(ok(true))
}
