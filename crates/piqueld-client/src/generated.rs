...
impl Client {
    ...
    */
    ...
    /*Sends a `PUT` request to `/api/v1/applications/{id}/services/{service}/secrets`
    ...
    - `body`
    */
    pub async fn set_service_secrets<'a>(
        &'a self,
        id: &'a str,
        service: &'a str,
        deploy: Option<bool>,
        ...
        force: Option<bool>,
        idempotency_key: Option<&'a str>,
        body: &'a piqueld_core::edit::SecretsValue,
    ) -> Result<
        ResponseValue<piqueld_core::api::Envelope<piqueld_core::api::SavedApplication>>,
        Error<piqueld_core::api::ErrorBody>,
    > {
        let url = format!(
            "{}/api/v1/applications/{}/services/{}/secrets",
            self.baseurl,
            encode_path(&id.to_string()),
            encode_path(&service.to_string()),
        );
        ...
        let mut request = self
            ...
            .build()?;
        let info = OperationInfo {
            operation_id: "set_service_secrets",
        };
        self.pre(&mut request, &info).await?;
        let result = self.exec(request, &info).await;
        self.post(&result, &info).await?;
