...
on core directly as product behavior arrives; persistence, Docker, API, and other
adapters remain internal daemon modules. Applications never become dependencies of
libraries.

Run `./scripts/check-dependency-boundaries.sh` after changing manifests. The script
is also part of the Nix flake checks.
